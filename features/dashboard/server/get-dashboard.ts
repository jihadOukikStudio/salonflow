...
async function completedServicesByEmployee(
  ...
  const map = new Map<
    string,
    { appointments: Set<string>; services: number }
  >();
  for (const row of rows) {
    if (!row.performedByEmployeeId) continue;
    const v = map.get(row.performedByEmployeeId) ?? {
      appointments: new Set<string>(),
      services: 0,
    };
    v.services++;
    v.appointments.add(row.appointmentId);
    map.set(row.performedByEmployeeId, v);
  }
  return [...map.entries()]
    .map(([employeeId, v]) => ({
      employeeId,
      name: employeeNames.get(employeeId) ?? "Employée",
      completedServices: v.services,
      appointmentCount: v.appointments.size,
    }))
    .sort(
      (a, b) =>
...
export async function getDashboard(currentUser: CurrentUser) {
  ...
  const operational = {
    ...
    teamActivity: { today: teamToday, week: teamWeek, month: teamMonth },
  };
