...
async function setup() {
  ...
  const employee = await testPrisma.employee.create({
    data: { salonId: salon.id, firstName: "Lina" },
  });
  ...
  const category = await testPrisma.serviceCategory.create({
    data: { salonId: salon.id, name: "Cheveux" },
  });
  const soin = await testPrisma.service.create({
    data: {
      salonId: salon.id,
...
describe("createPlannedAppointment — planning prestation par prestation", () => {
  ...
  it("autorise la même employée sur deux prestations consécutives du même RDV", async () => {
    const c = await setup();
    const result = await createPlannedAppointment(c.currentUser, {
      client: { type: "existing", clientId: c.client.id },
      services: [
        {
          serviceId: c.soin.id,
        ...
        },
        {
          ...
          scheduledStart: new Date("2099-09-10T11:00:00.000Z"),
          employeeId: c.employee.id,
        },
      ],
    });
    ...
    expect(
      result.services.every((s) => s.assignedEmployeeId === c.employee.id),
    ).toBe(true);
  });
  ...
  it("refuse la même employée si deux prestations du brouillon se chevauchent", async () => {
    const c = await setup();
    await expect(
      createPlannedAppointment(c.currentUser, {
        client: { type: "existing", clientId: c.client.id },
        services: [
          {
            serviceId: c.soin.id,
          ...
          {
            serviceId: c.brushing.id,
            scheduledStart: new Date("2099-09-10T10:45:00.000Z"),
            employeeId: c.employee.id,
          },
        ],
      }),
  ...
  it("autorise une salle à être réutilisée exactement à la fin de la prestation précédente", async () => {
    const c = await setup();
    const soin2 = await testPrisma.service.create({
      data: {
        salonId: c.salon.id,
        categoryId: c.soin.categoryId,
        name: "Soin visage",
        defaultDurationMinutes: 30,
        defaultPrice: 200,
        requiredRoomType: "TREATMENT_ROOM",
  ...
  it("refuse un créneau déjà occupé par un autre RDV", async () => {
    const c = await setup();
    const other = await testPrisma.client.create({
      data: { salonId: c.salon.id, name: "Autre", phone: "+212600000099" },
    });
    await testPrisma.appointment.create({
      data: {
        salonId: c.salon.id,
        clientId: other.id,
        scheduledStart: new Date("2099-09-10T10:30:00.000Z"),
        estimatedDurationMinutes: 30,
        createdByUserId: c.user.id,
        services: {
          create: {
            serviceNameSnapshot: "Occupée",
            durationMinutes: 30,
            scheduledStart: new Date("2099-09-10T10:30:00.000Z"),
            price: 100,
  ...
  it("conserve le prix catalogue comme base quand le prix du RDV est personnalisé", async () => {
    ...
    const result = await createPlannedAppointment(c.currentUser, {
      ...
      services: [
        {
          serviceId: c.brushing.id,
          scheduledStart: new Date("2099-09-10T12:00:00.000Z"),
          employeeId: c.employee.id,
          price: 190,
        },
      ],
  ...
  it("refuse seulement la prestation qui chevauche une indisponibilité dans un RDV multi-prestations", async () => {
    ...
    const secondEmployee = await testPrisma.employee.create({
      data: { salonId: c.salon.id, firstName: "Sara" },
    });
    await testPrisma.employeeUnavailability.create({
      data: {
        employeeId: c.employee.id,
    ...
    await expect(
      createPlannedAppointment(c.currentUser, {
        ...
        services: [
          {
            serviceId: c.brushing.id,
            scheduledStart: new Date("2099-09-10T10:07:00.000Z"),
            employeeId: c.employee.id,
          },
        ],
      }),
    ...
    await expect(
      createPlannedAppointment(c.currentUser, {
        ...
        services: [
          {
            serviceId: c.brushing.id,
            scheduledStart: new Date("2099-09-10T21:15:00.000Z"),
            employeeId: c.employee.id,
          },
        ],
      }),
  ...
  it("autorise une prestation qui finit exactement à 21h30", async () => {
    ...
    const result = await createPlannedAppointment(c.currentUser, {
      ...
      services: [
        {
          serviceId: c.brushing.id,
          scheduledStart: new Date("2099-09-10T21:00:00.000Z"),
          employeeId: c.employee.id,
        },
      ],
    });
