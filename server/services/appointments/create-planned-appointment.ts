...
export async function createPlannedAppointment(
  ...
  if (input.services.length === 0) {
    throw new BusinessRuleError(
      "Ajoutez au moins une prestation au rendez-vous.",
    );
  }
  ...
  return prisma.$transaction(
    async (tx) => {
      const serviceIds = [
        ...new Set(input.services.map((item) => item.serviceId)),
      ];
      ...
      const prepared = input.services.map((item) => {
        ...
        if (
          ...
          throw new BusinessRuleError(
            `La durée de « ${service.name} » doit être configurée.`,
          );
        }
        if (Number.isNaN(item.scheduledStart.getTime())) {
          throw new BusinessRuleError(
            `Le créneau de « ${service.name} » est invalide.`,
          );
        }
      ...
      for (let i = 0; i < prepared.length; i += 1) {
        for (let j = i + 1; j < prepared.length; j += 1) {
          ...
          if (a.item.employeeId === b.item.employeeId) {
            throw new BusinessRuleError(
              "Une même employée ne peut pas réaliser deux prestations qui se chevauchent.",
            );
          }
          if (a.item.roomId && a.item.roomId === b.item.roomId) {
            throw new BusinessRuleError(
              "Une même salle ne peut pas être utilisée par deux prestations qui se chevauchent.",
            );
          }
        }
      }
      ...
      if (input.client.type === "existing") {
        const client = await tx.client.findFirst({
          where: { id: input.client.clientId, salonId, isActive: true },
        });
        ...
        const client = await tx.client.upsert({
          ...
          update:
            input.client.clientNote !== undefined
      ...
      const ends = prepared.map(
        ({ item, durationMinutes }) =>
          item.scheduledStart.getTime() + durationMinutes * 60_000,
      );
      const scheduledStart = new Date(Math.min(...starts));
      const estimatedDurationMinutes = Math.ceil(
        (Math.max(...ends) - scheduledStart.getTime()) / 60_000,
      );
      ...
      const appointment = await tx.appointment.create({
        data: {
          ...
          services: {
            create: prepared.map(
              ({ item, service, durationMinutes, price, basePrice }) => ({
                serviceId: service.id,
