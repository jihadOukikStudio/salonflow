...
export async function updateAppointmentServiceComment(currentUser: CurrentUser, input: Input) {
  ...
  const service = await prisma.appointmentService.findFirst({
    where: { id: input.appointmentServiceId, appointment: { salonId: user.salonId } },
    select: { id: true, assignedEmployee: { select: { userId: true } }, appointment: { select: { status: true } } },
  });
  if (!service) throw new ResourceNotFoundError("La prestation demandée est introuvable.");
  if (service.appointment.status === "CANCELLED" || service.appointment.status === "CLOSED")
    throw new BusinessRuleError("Le commentaire de cette prestation ne peut plus être modifié.");
  const manager = user.role === "ADMIN" || (user.role === "EMPLOYEE" && user.canManageSalon);
